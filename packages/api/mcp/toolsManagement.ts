...
export function addManagementTools(tools: McpTool[], deps: ToolDeps, { todos, config, runtime }: Handlers): void {
  ...
  tools.push({ name: 'update_repository_configuration', description: 'Update branch, alias, enabled state, CI followup, notifications or visual preview policy for a specific configured repository.', scope: 'manage', permission: 'instance.manage_settings', schema: z.object({ ...mutationShape, repository: repositorySchema, baseBranch: idSchema.optional(), alias: idSchema.optional(), enabled: z.boolean().optional(), autoFollowupOnFailedCi: z.boolean().optional(), cancelCiDuringFollowup: z.boolean().optional(), notificationsEnabled: z.boolean().optional(), visualPreview: z.object({ enabled: z.boolean(), types: z.array(z.enum(['image', 'video'])).min(1).max(2), instructions: z.string().max(8192).optional() }).strict().optional() }).strict(), run: async ({ principal, args }) => {
    const repos = await loadMonitoredReposRaw();
    const patch = Object.fromEntries(['baseBranch', 'alias', 'enabled', 'autoFollowupOnFailedCi', 'cancelCiDuringFollowup', 'notificationsEnabled', 'visualPreview'].filter(key => args[key] !== undefined).map(key => [key, args[key]]));
