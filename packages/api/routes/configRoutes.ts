...
import { normalizeRepoConfig, preserveRepoAutoFollowup, preserveRepoCancelCiDuringFollowup, preserveRepoNotifications, preserveRepoVisualPreview } from './configRepoValidation.js';
...
export function createConfigRoutes(deps: ConfigRoutesDeps) {
  ...
  async function postRepos(req: Request, res: Response): Promise<void> {
    ...
    const result = await withConfigLock(redisClient, 'config:repos:lock', async lock => {
      ...
      const withPreservedNotifications = preserveRepoNotifications(previousRepos, preserveRepoCancelCiDuringFollowup(previousRepos, withPreservedAutoFollowup, repos_to_monitor), repos_to_monitor);
