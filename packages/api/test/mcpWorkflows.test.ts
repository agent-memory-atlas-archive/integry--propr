  ...
  try {
    ...
    for (const modern of [true, false]) {
      ...
      try {
        ...
        const review = await core.createNotificationEvent({ kind: 'review', severity: 'warning', deduplicationKey: `fixture-review-${modern}`, occurredAt: new Date(Date.now() - 3000), title: 'Retry transient failures', body: 'Score 7/10 · 1 issue', target: { type: 'review', repository, prNumber: 42 }, recipients: ['123'] });
        const system = await core.createNotificationEvent({ kind: 'system_failure', severity: 'error', deduplicationKey: `fixture-system-${modern}`, occurredAt: new Date(Date.now() - 2000), title: 'Worker unavailable', body: 'Queue processing stopped', target: { type: 'system_failure', component: 'worker' }, recipients: ['123'] });
        const foreign = await core.createNotificationEvent({ kind: 'task', deduplicationKey: `fixture-foreign-${modern}`, occurredAt: new Date(Date.now() - 1000), title: 'Outside grant', body: 'Hidden', target: { type: 'task', repository: 'other/repo', taskId: `foreign-${modern}` }, recipients: ['123'] });
        ...
        const resource = await client.readResource({ uri: `propr://instances/${config.instanceId}/notifications/${system.id}` });
        ...
        assert.equal((await core.notificationService.getNotification('123', foreign.id))!.dismissedAt, null);
