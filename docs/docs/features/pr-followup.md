...
- Only queued or running GitHub Actions runs that GitHub itself associates with that exact pull request and that exact head commit, and whose workflow is eligible validation, are cancelled. Manual, scheduled, branch, tag and other pull requests' runs, other revisions and non-Actions checks are never touched, and a branch name alone never qualifies a run.
- **Which workflows are eligible.** A trigger event is not a workflow classification, so the event alone never qualifies a run. By default a workflow is eligible when its run was triggered by `pull_request` — never by `pull_request_target`, the trigger preview and deployment automation uses, including this repository's own "PR Preview" — *and* its display name or workflow file names validation work (`ci`, `check`, `test`, `lint`, `build`, `compile`, `validate`, `verify`, `typecheck`, `unit`, `e2e`, `integration`, `compatibility`, `coverage`, `guard`, `review`, `audit`, `scan`, `analysis`, `codeql`, `quality`, `format`, `spell`, `security`) *and* names no publication work (`deploy`, `preview`, `publish`, `release-please`, `provision`, `promote`, `rollout`, `staging`, `production`, `pages`, `announce`, `upload`, `npm`, `dockerhub`). Anything else — a deployment workflow, or a workflow whose purpose its name does not state — keeps running.
- **Naming the workflows yourself.** Set `CANCEL_CI_FOLLOWUP_WORKFLOWS` to a comma-separated list to replace that default entirely: exactly the listed workflows become eligible and nothing else is ever cancelled. Entries match case-insensitively against a workflow's display name, its file path, or that file's name (`Build & Lint Check`, `.github/workflows/pr-build-check.yml`, `pr-build-check.yml` and `pr-build-check` all select the same workflow), and a workflow you list deliberately is eligible even when it runs on `pull_request_target`.
