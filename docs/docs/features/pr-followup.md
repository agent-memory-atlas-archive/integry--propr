...
- The obligation is stored durably before each cancel request is sent, so a crash or a lost response can never leave CI cancelled without a restart obligation; reconciliation reads each run's real outcome and restarts only what GitHub really cancelled. A denied retry never discards an obligation an earlier attempt already took on, and a restart GitHub refuses keeps its runs recorded in a blocked state that every later reconciliation retries — cancelled checks are released only once their restart is confirmed, the pull request closes or its head is replaced. It also picks up runs that GitHub queued after the cancellation, and releases the suspension if you switch the option off mid-task.
