...
4. **Recent outcomes** — a flat feed of results, last 24 hours by default with a seven-day option. A quality score appears only where one was recorded, drawn as the fixed-width quality pill (`[ ● 9 ]`, `[ ◆ 7 ]`); the out-of-ten scale is announced to assistive technology rather than printed as `/10`, which would put variable-width glyphs outside the badge and make the right rail shift between rows.
