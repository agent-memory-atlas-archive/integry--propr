...
| `CANCEL_CI_FOLLOWUP_WORKFLOWS` | Unset (documented default policy) | Comma-separated workflows whose runs the per-repository option *Cancel CI while follow-up implementation is in progress* may cancel, matched case-insensitively by display name, workflow file path or file name. When set, only these workflows are ever cancelled — including `pull_request_target` ones named deliberately. Unset keeps the default: `pull_request` runs of workflows whose name or file identifies validation and not publication. See [PR Follow-up](../features/pr-followup.md#cancelling-obsolete-checks-during-follow-up). | Optional; only with that option enabled. |
